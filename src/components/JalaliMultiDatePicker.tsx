    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        {value.length > 0 && (
          ...
          >
            ...
            پاک کردن تاریخ‌ها ({value.length})
          </Button>
        )}
      </div>
      ...
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5 pt-1">
          {dateObjects.map((d, index) => {
            const jalaliStr = d.convert(persian, persian_fa).format('YYYY/MM/DD');
            const gStr = value[index];
            return (
              <span
                key={gStr || index}
              ...
              >
                ...
                <span>{jalaliStr}</span>
                <span className="text-[10px] text-muted-foreground">({gStr})</span>
              </span>
            );
          })}
        </div>
